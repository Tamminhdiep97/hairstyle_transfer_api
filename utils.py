...
from super_image import AwsrnModel, MdsrModel, EdsrModel, ImageLoader

gpu_device = torch.device('cuda')
cpu_device = torch.device('cpu')
fa = FaceAlignment(LandmarksType.TWO_D, device='cuda:0')

# upscale_model = EdsrModel.from_pretrained('eugenesiow/edsr-base', data_parallel=False, scale=2)
# upscale_model = upscale_model.to(cpu_device)
upscale_model = AwsrnModel.from_pretrained('eugenesiow/awsrn-bam', scale=2).to(gpu_device)
# upscale_model = MdsrModel.from_pretrained('eugenesiow/mdsr', scale=4).to(gpu_device)


async def upscale_result(image):
    # image = image.to(cpu_device)
    result = upscale_model(image)
    return result
...
def load_and_split_image(path, return_mask = False):
    ...
    if os.path.exists(hair_path_with_filename):
        ...
        print(inner_f_im.shape)
        imageio.imwrite(hair_path_with_filename, hair_im)
        imageio.imwrite(face_path_with_filename, inner_f_im.astype(np.uint8))
...
def detectFace_splitImage(image):
    inner_f_im, hair_im = split_image(image)
    return inner_f_im, hair_im
...
def prepare_dataset(path):
    ...
    for fil in image_files:
        ...
        imageio.imwrite(os.path.join(path,'inner_face', name), inner_f_im.astype(np.uint8))
