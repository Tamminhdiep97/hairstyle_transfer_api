...
async def run_inference(tool, args):  # -> asyncio.coroutine:
    if args['mode'] == 'transfer':
        paths_to_results = tool.hairstyle_transfer(args['source'], args['target'], alpha_blend = args['alpha_blend'])
    elif args['mode'] == 'interp':
        paths_to_results = await asyncio.gather(tool.interpolation_single_pair(args['source'], args['target'], n_steps=args['n_steps_interp'], interpolate_hair = not args['face_interp'], alpha_blend = args['alpha_blend']))
...
if __name__ == '__main__':
    args = parse_args()
    logger.info(args['alpha_blend'])
    logger.info('All done')
    t_0 = time.time()
    result_path = args['path']
    tool = Tool(opts=None, result_path=result_path, checkpoint_path='./best_model.pt')
    paths_to_results = asyncio.run(run_inference(tool, args))
    t_1 = time.time()
    logger.info('Time execute: {} s'.format(str(round(t_1-t_0, 4))))
